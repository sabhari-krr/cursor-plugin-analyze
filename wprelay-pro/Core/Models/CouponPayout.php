...
class CouponPayout extends Model
{
    ...
    public static function removeCouponUsage($payout)
    {
        if (strtolower($payout->payment_source) != 'coupon') {
            return;
        }

        $couponPayout = CouponPayout::where('payout_id = %d', [$payout->id])
            ->first();

        if (empty($couponPayout)) {
            return;
        }

        $coupon = new \WC_Coupon($couponPayout->coupon_code);
        $coupon->delete();
    }
}
