...
namespace RelayWp\Affiliate\Core\Controllers\Admin\HooksController\WooCommerce;
...
use RelayWp\Affiliate\App\Notifications\Emails\AffiliateNewSaleMadeEmail;
...
class EmailController
{
    public static function addEmails($emails)
    {
        ...
        if(!isset($emails['AffiliateNewSaleMadeEmail']) && class_exists(AffiliateNewSaleMadeEmail::class)){
            $emails['AffiliateNewSaleMadeEmail'] = new AffiliateNewSaleMadeEmail();
        }
    ...
    public static function sendAffiliateNewSaleMadeEmail($data, $order_id)
    {
        try {
            \WC_Emails::instance();
            ...
            if (isset($emails['AffiliateNewSaleMadeEmail'])) {
                $emails['AffiliateNewSaleMadeEmail']->trigger($data, $order_id);
            }
        } catch (\Error $error) {
            PluginHelper::logError('Error Occurred While Sending Affiliate Rejected Email', [__CLASS__, __FUNCTION__], $error);
        }
