...
class AccountController
{
        ...
        if ($user) {
            ...
            if ($member) {
                ...
                if ($affiliate && $affiliate->status != 'rejected') {
                    ...
                    $menu = array_slice($menu, 0, $lastIndex) + [$endpoint_name => $pluginName] + array_slice($menu, $lastIndex);
                    return $menu;
                }
            } else {
                return $menu;
    ...
    public static function registerAffiliateEndPoint()
    {
        ...
        add_rewrite_endpoint($endpoint_name, EP_ROOT | EP_PAGES);
    }
    ...
    public static function registerAffiliateEndpointContent()
    {
		...
		$allData['inside'] = $allData;
        wc_get_template('wprelay-menu.php', $allData, RWPA_TEMPLATE_OVERRIDE_DIR_PATH, $path);
    }
