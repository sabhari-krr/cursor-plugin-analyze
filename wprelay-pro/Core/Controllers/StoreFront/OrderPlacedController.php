...
class OrderPlacedController
{
    ...
    public static function orderStatusUpdated($order_id)
    {
        ...
        if(!$order->get_meta(Affiliate::ORDER_PLACED_EMAIL_SEND,true)){
            static::sendEmailsForSaleMade($order,$affiliate);
        }
    ...
    public static function sendEmailsForSaleMade($order,$affiliate){
        $member = Member::query()->find($affiliate->member_id);
        $order_id = $order->get_id();
        ...
        $new_sale_made_email_for_affiliate = apply_filters('rwpa_affiliate_new_sale_made_email_enabled', Settings::get('email_settings.affiliate_emails.affiliate_new_sale_made'));
        ...
        if ($new_sale_made_email) {
            do_action('rwpa_send_new_sale_made_email', $email_data, $order_id);
        }

        if ($new_sale_made_email_for_affiliate) {
            do_action('rwpa_send_new_sale_made_affiliate_email', $email_data, $order_id);
        }

        $order->update_meta_data(Affiliate::ORDER_PLACED_EMAIL_SEND, true);
        $order->save_meta_data();
    }
}
