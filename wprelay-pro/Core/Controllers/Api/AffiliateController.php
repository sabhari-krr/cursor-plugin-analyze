    ...
    public function updateStatus(Request $request)
    {
        ...
        try {
            ...
            if (is_null($affiliate->referral_code)) {
                ...
                $coupon_code = Affiliate::getCouponCode($settingsOption,$member);

                $is_coupon_exists = WC::isCouponExists($coupon_code);

                if($is_coupon_exists){
                    $coupon_code = Affiliate::getCouponCode('random',$member);
                    $is_coupon_exists = WC::isCouponExists($coupon_code);
                }
            ...
            if ($createWCAccount && $createWCAccount != 'false' && empty($affiliate->wc_customer_id)) {
                $user_id = Affiliate::createWCAccount($member, $affiliate);
                if (!empty($user_id)) {
                    $wp_user_id = $user_id;
