...
class CompatibleCheck {
	...
	public static function checkDependencies(bool $allow_exit = false): bool
	{
		if (!self::isPHPCompatible()) {
			...
			$message = sprintf(esc_html__('%1$s requires minimum PHP version %2$s', 'wployalty-point-email-reminder'), WLPR_PLUGIN_NAME, WLPR_MINIMUM_PHP_VERSION);
		...
		if (!self::isWordPressCompatible()) {
			...
			$message = sprintf(esc_html__('%1$s requires minimum WordPress version %2$s', 'wployalty-point-email-reminder'), WLPR_PLUGIN_NAME, WLPR_MINIMUM_WP_VERSION);
		...
		if (!self::isActive('woocommerce/woocommerce.php')) {
			...
			$message = sprintf(esc_html__('%1$s requires WooCommerce to be installed and activated in order to be used.', 'wployalty-point-email-reminder'), WLPR_PLUGIN_NAME);
			$allow_exit ? exit(esc_html($message)) : self::adminNotice(esc_html($message), 'error');
		...
		if (!self::isWooCompatible()) {
			...
			$message = sprintf(esc_html__('%1$s requires minimum Woocommerce version %2$s', 'wployalty-point-email-reminder'), WLPR_PLUGIN_NAME, WLPR_MINIMUM_WC_VERSION);
			$allow_exit ? exit(esc_html($message)) : self::adminNotice(esc_html($message), 'error');
		...
		if ((!self::isActive('wployalty/wp-loyalty-rules-lite.php')) && (!self::isActive('wp-loyalty-rules/wp-loyalty-rules.php'))) {
			...
			$message = sprintf(esc_html__(
				'%1$s requires WPLoyalty to be installed and activated in order to be used.',
		...
		if (!self::isLoyaltyCompatible()) {
			// translators: 1. %s will replace plugin name, 2. %s replace WooCommerce version
			$message = sprintf(esc_html__('%1$s requires minimum WPLoyalty version %2$s', 'wployalty-point-email-reminder'), WLPR_PLUGIN_NAME, WLPR_MINIMUM_WLR_VERSION);
			$allow_exit ? exit(esc_html($message)) : self::adminNotice(esc_html($message), 'error');
	...
	{
		return (int) version_compare(PHP_VERSION, WLPR_MINIMUM_PHP_VERSION, '>=') > 0;
	}
	...
	{
		return (int) version_compare(get_bloginfo('version'), WLPR_MINIMUM_WP_VERSION, '>=') > 0;
	}
	...
	protected static function isWooCompatible(): bool
	{
		...
		return (int) version_compare($woo_version, WLPR_MINIMUM_WC_VERSION, '>=') > 0;
	}
	...
	protected static function isLoyaltyCompatible(): bool
	{
		$wlr_version = self::getWLRVersion();
			...
			return (int) version_compare($wlr_version, WLPR_MINIMUM_WLR_VERSION, '>=') > 0;
	}
